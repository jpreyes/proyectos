...
/**
 * Same five destinations at both widths. On a phone they sit in a floating bar
...
export function TabBar({ open }: { open: number }) {
  ...
  return (
    <nav
      ...
      className="bottom-tabbar fixed inset-x-0 z-40 mx-auto flex w-fit max-w-[calc(100vw-1.5rem)] items-center gap-0.5 rounded-full bg-panel/95 p-1.5 shadow-lg shadow-black/40 backdrop-blur md:hidden"
    >
      {TABS.map((t) => {
        ...
        return (
          <Link
            ...
            className={cx(
              "relative flex min-w-0 flex-col items-center gap-1 rounded-full px-3 py-2 transition-colors",
              active ? "bg-pill" : "active:bg-pill/50"
            )}
          >
            <span
              className={cx("text-[17px] leading-none", active ? "text-accent" : "text-muted")}
              aria-hidden
            >
              {t.icon}
            </span>
            ...
            {n > 0 && (
              <span className="absolute right-1.5 top-1 grid h-4 min-w-4 place-items-center rounded-full bg-warn px-1 text-[10px] font-bold leading-none text-bg">
                {n}
              </span>
            )}
          </Link>
